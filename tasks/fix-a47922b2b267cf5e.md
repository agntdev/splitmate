# fix-a47922b2b267cf5e — /list sorts alphabetically by name, not by timestamp descending as spec requires

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SPLT

The E2T2 task spec states: "display all saved expenses for the user in **descending timestamp order**." The implementation at `E2T2.ts:34` and `E2T2.ts:54` sorts entries by `b.name.localeCompare(a.name)` — alphabetical by expense name in reverse order. No timestamp is stored or used anywhere; the expense record model (`ExpenseEntry`) has only `name` and `amount` fields. The list output order does not match the spec.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-a47922b2b267cf5e.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-a47922b2b267cf5e.json` (and any new command `tests/commands/fix-a47922b2b267cf5e.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-a47922b2b267cf5e.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-a47922b2b267cf5e.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
