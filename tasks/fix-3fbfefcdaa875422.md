# fix-3fbfefcdaa875422 — Each handler independently creates its own ioredis client (resource waste)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SPLT

E2T1, E2T2, and E2T3 each call `getStore()` which independently instantiates a new ioredis client via `createRequire('ioredis')`. Since all three handlers share the same key namespace (`expense:<chatId>:<name>`), a single shared client (or shared store module) would suffice. Three separate connections to the same Redis instance wastes resources. The toolkit's `defaultRedisStorage` already demonstrates the correct pattern for a shared Redis client.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-3fbfefcdaa875422.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-3fbfefcdaa875422.json` (and any new command `tests/commands/fix-3fbfefcdaa875422.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-3fbfefcdaa875422.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-3fbfefcdaa875422.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
