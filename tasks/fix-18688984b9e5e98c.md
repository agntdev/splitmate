# fix-18688984b9e5e98c — In-memory Map fallback violates the implementation contract

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SPLT

All three handlers fall back to a `Map`-based store (`MemoryExpenseStore`, `MemoryListStore`, `MemoryClearStore`) when `REDIS_URL` is not set. The task spec explicitly states: "No in-memory data store: a Map/array/module-level variable used as a database is a defect. Anything that must survive a restart MUST use the toolkit's persistent storage (Redis-backed), not process memory." The toolkit already provides `RedisSessionStorage`, `defaultRedisStorage`, and `resolveSessionStorage` patterns for Redis-backed storage — the handlers should use those (or a shared domain-data equivalent), not invent their own in-memory fallback.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-18688984b9e5e98c.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-18688984b9e5e98c.json` (and any new command `tests/commands/fix-18688984b9e5e98c.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-18688984b9e5e98c.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-18688984b9e5e98c.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
