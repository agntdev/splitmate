# fix-edf01c0346cebcd7 — Duplicate /start command — two composers register the same handler, causing double replies

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SPLT

`src/handlers/E3T1.ts` registers `composer.command("start", ...)` but `src/handlers/start.ts` (shipped by T01) also registers the same command. Both files are auto-loaded by `buildBot()` at `src/bot.ts:39-45`, so grammY fires both middlewares when a user sends `/start` — the user receives **two** welcome messages (one with the inline keyboard from `start.ts`, one with the plain text from `E3T1.ts`). The E3T1 task spec does not mention replacing or coexisting with an existing `/start` handler, but the implementation creates a concrete runtime conflict.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-edf01c0346cebcd7.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-edf01c0346cebcd7.json` (and any new command `tests/commands/fix-edf01c0346cebcd7.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-edf01c0346cebcd7.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-edf01c0346cebcd7.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
