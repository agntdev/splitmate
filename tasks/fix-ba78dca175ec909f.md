# fix-ba78dca175ec909f — `/help` command missing `/split`, `/save`, `/list`, `/clear` — incomplete command reference

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SPLT

`src/handlers/help.ts` lists only `/start`, `/t01`, and `/help`, but the bot supports five additional commands (`/split`, `/save`, `/list`, `/clear`). Users have no in-app way to discover these commands. The help handler should enumerate all registered commands.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-ba78dca175ec909f.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-ba78dca175ec909f.json` (and any new command `tests/commands/fix-ba78dca175ec909f.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-ba78dca175ec909f.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-ba78dca175ec909f.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
